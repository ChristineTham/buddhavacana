...
This is a subversive sutra, for it "cuts" through "traditional" or "conservative" interpretations of the Buddha's teachings, as represented by the Hīnayāna scriptures (the "inferior way"). It goes through a series of apparent contradictions discussed in dialogues between the Buddha and his disciple Subhūti. These dialogues demolish the idea of inherent existence, and shows that all phenomena are empty of a permanent, independent self-nature (`śūnyatā`). They also emphasise the importance of non-attachment and the practice of compassion towards all beings (because these beings are ultimately lacking in inherent existence as well, our perceptions of them are manifestations).
...
- Giving gifts without being based or dependent on the five collections (skhandhas). Acts of "kindness" must be intrinsically "good" and not attached to any perception of self, other, or phenomena.
...
- The understanding of `dharma` itself as well as the generation of "Buddha field" (`kṣetravyūhān`).
...
> And furthermore, Subhūti, if a woman or a man were to renounce self-bodies equal to the sands of the river Ganges, and if someone, having learned from this `dharmaparyāya` (method of teaching) at least a four-lined verse, should teach it to others, this one on that account would generate much `puṇya` (merit), immeasurable, innumerable.
...
> How, Bhagavān, should one who has set out in the `bodhisatvayāna` (`bodhisatvayāna`-way) stand, how should one practice, how should one control the mind?
...
> What do you think, Subhūti, does the `Tathāgata` possess the `divyaṃ cakṣuḥ` (divine eye), the `prajñācakṣuḥ` (wisdom eye), the `dharmacakṣuḥ` (Dharma eye), the `buddhacakṣuḥ` (Buddha eye)? He said, so it is, Bhagavān, the `Tathāgata` possesses the `divyaṃ cakṣuḥ`, the `prajñācakṣuḥ`, the `dharmacakṣuḥ`, the `buddhacakṣuḥ`.
